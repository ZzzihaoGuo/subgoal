...
def manifold_single_agent_(
    ...
    Returns
    -------
    u_opt : Array, shape (dim_q,)  — action 空间
    relax : Array, shape (k,)
    s_new : Array, shape (n_total,)
    """
    n_agent = len(a_state)

    q = state[:dim_q]   # position
    dq = state[dim_q:]  # velocity

    all_obs_state = jnp.concatenate([a_state, o_obs_state], axis=0)
    all_obs_pos = all_obs_state[:, :dim_q]
    all_obs_vel = all_obs_state[:, dim_q:]  # (n_total, dim_q)
...
def manifold_all_agents(
    ...
    """对所有 agent 并行计算 manifold 修正 (支持 2D/3D)"""
    ...
    if get_pos_acc_jacobian is not None:
        a_G = get_pos_acc_jacobian(a_states_raw)  # (n_agent, dim_q, dim_q)
    else:
        a_G = jnp.broadcast_to(acc_scale * jnp.eye(dim_q), (n_agent, dim_q, dim_q))

    # convert to [pos, vel] for manifold computation
    ...
    fn = jax.vmap(
        ft.partial(manifold_single_agent_, r=r, k=k, K=K, Kc=Kc, s_min=s_min,
                   ...
                   acc_scale=acc_scale, w_slack=w_slack, dim_q=dim_q),
...
def manifold_init_slack(
    ...
    def _init_single(state, agent_idx, o_obs_state, a_state):
        ...
        all_obs_state = jnp.concatenate([a_state, o_obs_state], axis=0)
        all_obs_pos = all_obs_state[:, :dim_q]
        all_obs_vel = all_obs_state[:, dim_q:]
        all_isobs = jnp.arange(len(all_obs_pos)) >= n_agent
        # 动态安全半径: 基于接近速度的刹车距离
...
def get_manifold_fn(env: MultiAgentEnv, k: int = 3, K: float = 0.15, Kc: float = 8.0,
                    ...
                    state_to_pos_vel=None, get_pos_acc_jacobian=None, dim_q: int = 2):
    """工厂函数：创建 manifold 修正函数 (支持 2D/3D via dim_q)"""
    n_agent = env.num_agents
    n_rays = env.params["top_k_rays"]
    r = env.params["car_radius"]
    dt = env._dt
    ...
    print(f"  manifold k clamped to {k} (max_neighbors={max_neighbors}), dim_q={dim_q}")
...
# ===================== VMAS Manifold (agent-agent + agent-obstacle) =====================

def manifold_single_agent_vmas_(
    state: Array,
    agent_idx: int,
    a_state: Array,
    o_state: Array,
    ...
    """ATACOM manifold for VMAS: agent-agent + agent-obstacle collision avoidance

    Parameters
    ----------
    state: (4,) [x, y, vx, vy] of this agent
    agent_idx: int, index of this agent
    a_state: (n_agent, 4) all agent states
    o_state: (n_obs, 4) all obstacle states [x, y, 0, 0] (static)
    u_ref: (2,) nominal action
    s_prev: (n_agent + n_obs,) slack variables for all neighbors
    ...
    # 拼接所有邻居: agents + obstacles
    all_obs_state = jnp.concatenate([a_state, o_state], axis=0)  # (n_agent + n_obs, 4)
    all_obs_pos = all_obs_state[:, :2]
    all_obs_vel = all_obs_state[:, 2:]
    n_total = n_agent + n_obs

    # 碰撞半径: agent-agent = 2*agent_r, agent-obs = agent_r + obs_r
    is_obs = jnp.arange(n_total) >= n_agent
    base_r = jnp.where(is_obs, agent_r + obs_r, 2 * agent_r)

    # 距离计算 (排除自己)
    o_dist_sq_now = ((q - all_obs_pos) ** 2).sum(axis=-1)
    o_dist_sq_now = o_dist_sq_now.at[agent_idx].set(1e6)
    all_rel_vel = dq[None, :] - all_obs_vel
    lookahead_sel = dt * jnp.maximum(n_lookahead, 2)
    o_p_rel_pred = (q - all_obs_pos) + all_rel_vel * lookahead_sel
    o_dist_sq_pred = (o_p_rel_pred ** 2).sum(axis=-1)
    o_dist_sq_pred = o_dist_sq_pred.at[agent_idx].set(1e6)
    o_dist_sq = jnp.minimum(o_dist_sq_now, o_dist_sq_pred)
    k_idx = jnp.argsort(o_dist_sq)[:k]

    k_p_rel = q - all_obs_pos[k_idx]
    k_vel = all_obs_vel[k_idx]
    k_isobs = k_idx >= n_agent
    k_rel_vel = dq[None, :] - k_vel

    # 动态安全半径
    k_p_rel_norm = jnp.linalg.norm(k_p_rel, axis=-1, keepdims=True)
    k_p_rel_hat = k_p_rel / (k_p_rel_norm + 1e-8)
    k_v_approach = jnp.maximum(0.0, -jnp.sum(k_rel_vel * k_p_rel_hat, axis=-1))
    k_effective_decel = jnp.where(k_isobs, acc_scale, 2.0 * acc_scale)
    k_braking_dist = k_v_approach ** 2 / (2.0 * k_effective_decel + 1e-8)
    k_dynamic_margin = safety_margin + k_braking_dist

    k_base_r = base_r[k_idx]
    k_safety_dist_sq = (k_base_r + k_dynamic_margin) ** 2

    # 约束值
    k_g_now = k_safety_dist_sq - (k_p_rel ** 2).sum(axis=-1)
    lookahead_dt = dt * n_lookahead
    k_p_rel_pred = k_p_rel + k_rel_vel * lookahead_dt
    k_g_pred = k_safety_dist_sq - (k_p_rel_pred ** 2).sum(axis=-1)
    k_g = jnp.maximum(k_g_now, k_g_pred)

    # Jacobian
    k_J_g = -2.0 * k_p_rel

    # Viability
    k_dg_dt = jnp.sum(k_J_g * k_rel_vel, axis=-1)
    k_K = jnp.where(k_isobs, K, K * 0.5)  # agent-agent 用更小的 K
    k_g_viab = k_g + k_K * k_dg_dt
    ...
    viab_active = (jnp.clip(k_g_viab / g_act_thresh, 0.0, 1.0)
                   * (k_g > -g_act_thresh * 10).astype(jnp.float32))
    active_f = jnp.maximum(pos_active, viab_active)

    # 增广 Jacobian
    G = acc_scale * jnp.eye(2)
    Jc_u = K * (k_J_g @ G) * active_f[:, None]
    Jc_slack = jnp.diag(k_s) * active_f[:, None] + jnp.eye(k) * (1.0 - active_f[:, None])
    Jc = jnp.concatenate([Jc_u, Jc_slack], axis=1)

    # psi
    k_dJdt_dq = -2.0 * jnp.sum(k_rel_vel * k_rel_vel, axis=-1)
    k_psi_raw = k_dg_dt + K * k_dJdt_dq
    k_psi = jnp.maximum(k_psi_raw, 0.0) * active_f
    ...
    lambda_sq = 0.01
    S_inv = S / (S ** 2 + lambda_sq)
    Jc_w_pinv = (Vh[:k].T * S_inv[None, :]) @ U.T
    Jc_pinv = W_inv[:, None] * Jc_w_pinv
    Nc_w = Vh[k:].T
    Nc = W_inv[:, None] * Nc_w

    # ATACOM 特解
    a_comp = -Jc_pinv @ k_psi
    k_c = jnp.maximum(k_g_viab, 0.0) * active_f
    err = -Jc_pinv @ (Kc * k_c)

    # Null space
    Nc_q = Nc[:dim_q]
    target = u_ref - (a_comp[:dim_q] + err[:dim_q])
    alpha, _, _, _ = jnp.linalg.lstsq(Nc_q, target)
    alpha_norm = jnp.linalg.norm(alpha)
    alpha = jnp.where(alpha_norm > alpha_max, alpha * alpha_max / (alpha_norm + 1e-8), alpha)
    b_proj = Nc @ alpha

    # 法向/切向分解
    normal_sum = jnp.sum(k_J_g * active_f[:, None], axis=0)
    normal_norm = jnp.linalg.norm(normal_sum)
    normal_dir = normal_sum / (normal_norm + 1e-8)
    b_action = b_proj[:dim_q]
    b_normal_coeff = jnp.dot(b_action, normal_dir)
    b_tangent = b_action - b_normal_coeff * normal_dir
    b_normal_safe = jnp.where(b_normal_coeff > 0, 0.1 * b_normal_coeff, b_normal_coeff)
    b_action_safe = b_tangent + b_normal_safe * normal_dir
    has_active = normal_norm > 1e-6
    ...
    # 更新所有邻居的松弛变量
    all_p_rel_all = q - all_obs_pos
    all_rel_vel_all = dq[None, :] - all_obs_vel
    all_J_g = -2.0 * all_p_rel_all
    all_dg_dt = jnp.sum(all_J_g * all_rel_vel_all, axis=-1)
    all_p_rel_norm = jnp.linalg.norm(all_p_rel_all, axis=-1, keepdims=True)
    all_p_rel_hat = all_p_rel_all / (all_p_rel_norm + 1e-8)
    all_v_approach = jnp.maximum(0.0, -jnp.sum(all_rel_vel_all * all_p_rel_hat, axis=-1))
    all_effective_decel = jnp.where(is_obs, acc_scale, 2.0 * acc_scale)
    all_braking_dist = all_v_approach ** 2 / (2.0 * all_effective_decel + 1e-8)
    all_dynamic_margin = safety_margin + all_braking_dist
    all_safety_sq = (base_r + all_dynamic_margin) ** 2
    all_g_now = all_safety_sq - (all_p_rel_all ** 2).sum(axis=-1)
    all_p_rel_pred = all_p_rel_all + all_rel_vel_all * lookahead_dt
    all_g_pred = all_safety_sq - (all_p_rel_pred ** 2).sum(axis=-1)
    all_g = jnp.maximum(all_g_now, all_g_pred)
    all_K = jnp.where(is_obs, K, K * 0.5)
    all_g_viab = all_g + all_K * all_dg_dt
    all_s_ideal = jnp.sqrt(jnp.maximum(-2.0 * all_g_viab, s_min ** 2))
    s_new = all_s_ideal
    s_new = s_new.at[k_idx].set(k_s_new)
    ...
    debug_info = jnp.array([
        u_ref[0], u_ref[1],
        u_opt[0], u_opt[1],
        jnp.max(k_g_viab * active_f),
        jnp.max(active_f),
        jnp.min(o_dist_sq[k_idx]),
        jnp.max(k_c),
    ])
    return u_opt, relax, s_new, debug_info


def manifold_all_agents_vmas(
    graph: GraphsTuple,
    u_ref: Array,
    s_all: Array,
    agent_r: float,
    ...
    """VMAS: 对所有 agent 并行计算 manifold 修正 (agent-agent + agent-obstacle)

    get_agent_state: callable(graph) -> (n_agent, 4), 从 graph.env_states 提取 agent [x,y,vx,vy]
    get_obs_state: callable(graph) -> (n_obs, 4), 从 graph.env_states 提取 obstacle [x,y,vx,vy]
    """
    # agent states from env_state
    a_states = get_agent_state(graph)  # (n_agent, 4)

    # obstacle states from env_state
    o_states = get_obs_state(graph)  # (n_obs, 4)

    agent_idx = jnp.arange(n_agent)
    fn = jax.vmap(
        ft.partial(manifold_single_agent_vmas_, agent_r=agent_r, obs_r=obs_r,
                   k=k, K=K, Kc=Kc, s_min=s_min,
                   alpha_max=alpha_max, g_act_thresh=g_act_thresh, dt=dt,
                   safety_margin=safety_margin, n_lookahead=n_lookahead,
                   acc_scale=acc_scale, w_slack=w_slack),
        in_axes=(0, 0, None, None, 0, 0)
    )
    u_opt, relax, s_new, debug_info = fn(a_states, agent_idx, a_states, o_states, u_ref, s_all)
    return u_opt, relax, s_new, debug_info


def manifold_init_slack_vmas(
    graph: GraphsTuple,
    agent_r: float,
    obs_r: float,
    n_agent: int,
    ...
    """VMAS: 初始化松弛变量 (agent-agent + agent-obstacle)"""
    a_states = get_agent_state(graph)  # (n_agent, 4)
    o_states = get_obs_state(graph)  # (n_obs, 4)
    ...
    def _init_single(state, agent_idx, a_state, o_state):
        q = state[:2]
        dq = state[2:]
        all_obs_state = jnp.concatenate([a_state, o_state], axis=0)
        all_obs_pos = all_obs_state[:, :2]
        all_obs_vel = all_obs_state[:, 2:]
        all_p_rel = q - all_obs_pos
        all_rel_vel = dq[None, :] - all_obs_vel
    ...
    agent_idx = jnp.arange(n_agent)
    s_all = jax.vmap(_init_single, in_axes=(0, 0, None, None))(
        a_states, agent_idx, a_states, o_states
    )
    return s_all


def get_manifold_fn_vmas(env, k: int = 3, K: float = 0.2, Kc: float = 15.0,
                         s_min: float = 0.1, alpha_max: float = 5.0, g_act_thresh: float = 0.1,
                         safety_margin: float = 0.02, n_lookahead: int = 2, w_slack: float = 5.0,
                         get_obs_state=None, get_agent_state=None):
    """VMAS 专用 manifold 工厂 (agent-agent + agent-obstacle)

    Parameters
    ----------
    env: VMAS environment (需要 .num_agents, .agent_radius, .obs_radius, .n_obs, ._dt)
    get_agent_state: callable(graph) -> (n_agent, 4), 从 graph.env_states 提取 agent [x,y,vx,vy]
    get_obs_state: callable(graph) -> (n_obs, 4), 从 graph.env_states 提取 obstacle [x,y,vx,vy]
    """
    n_agent = env.num_agents
    n_obs = env.n_obs
    agent_r = env.agent_radius
    obs_r = env.obs_radius
    dt = env._dt
    acc_scale = 0.5  # u_multiplier / mass = 0.5 / 1.0
    max_neighbors = (n_agent - 1) + n_obs
    k = min(k, max_neighbors)
    print(f"  VMAS manifold k={k} (max_neighbors={max_neighbors})")
    print(f"  VMAS manifold params: K={K}, Kc={Kc}, alpha_max={alpha_max}, "
          f"g_act_thresh={g_act_thresh}, safety_margin={safety_margin}, "
          f"dt={dt}, acc_scale={acc_scale}, n_lookahead={n_lookahead}")

    manifold_fn = ft.partial(
        manifold_all_agents_vmas,
        agent_r=agent_r, obs_r=obs_r, n_agent=n_agent, n_obs=n_obs,
        k=k, K=K, Kc=Kc, s_min=s_min,
        alpha_max=alpha_max, g_act_thresh=g_act_thresh, dt=dt,
        safety_margin=safety_margin, n_lookahead=n_lookahead,
        acc_scale=acc_scale, w_slack=w_slack,
        get_obs_state=get_obs_state, get_agent_state=get_agent_state,
    )

    init_slack_fn = ft.partial(
        manifold_init_slack_vmas,
        agent_r=agent_r, obs_r=obs_r, n_agent=n_agent, n_obs=n_obs,
        K=K, s_min=s_min, safety_margin=safety_margin, dt=dt,
        n_lookahead=n_lookahead, acc_scale=acc_scale,
        get_obs_state=get_obs_state, get_agent_state=get_agent_state,
    )

...
from .utils import test_rollout_subgoal_manifold_vmas
from .utils import rollout_hierarchical_manifold_vmas
...
class Trainer:
    ...
    def train(self):
        ...
        # === Train collect: VMAS manifold rollout ===
        def collect_fn_single(cur_params, cur_key, reach_thresh):
            return rollout_hierarchical_manifold_vmas(
                self.env,
                ft.partial(self.algo.step, params=cur_params),
                init_rnn_state,
                cur_key,
                subgoal_interval=subgoal_interval,
                reach_thresh=reach_thresh,
        ...
        def collect_fn(cur_params, cur_keys, reach_thresh):
            return jax.vmap(ft.partial(collect_fn_single, cur_params, reach_thresh=reach_thresh))(cur_keys)
        ...
        reach_thresh_schedule_fn = getattr(self.algo, 'reach_thresh_schedule_fn', lambda x: 0.01)
        ...
        for step in range(0, self.steps + 1):
            ...
            if step % self.eval_interval == 0:
                ...
                # sparse_rewards 统计 (VMAS: sparse_reward = env reward)
            ...
            # collect rollouts with VMAS manifold safety
            key_x0, self.key = jax.random.split(self.key)
            key_x0 = jax.random.split(key_x0, self.n_env_train)
            reach_thresh = reach_thresh_schedule_fn(step)
            rollouts = collect_fn(self.algo.params, key_x0, reach_thresh)
